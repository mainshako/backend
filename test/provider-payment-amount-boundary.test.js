...
for (const amount of [0, -1, Number.NaN, Number.POSITIVE_INFINITY, 'not-a-number', '0.004']) {
  test(`invalid payment amount ${String(amount)} fails closed before provider network`, async () => {
    let networkCalls = 0;
    ...
    await assert.rejects(
      provider.createPayment({ amount, currency: 'ILS', merchantTransactionId: 'order_12345678' }),
